...
void SimpleCanvas::drawAxes(cairo_t* cr) {
    ...
    s21::Axes axes;
    s21::matrix_t m = axes.getMatrix();
    for (const auto& i : m) {
        std::cout << std::format("{},{},{}\n", i[0], i[1], i[2]);
    }
    m = s21::Transformer::Rotate(c_->getAngleX(), c_->getAngleY(), c_->getAngleZ(), axes.getMatrix());
    for (const auto& i : m) {
        std::cout << std::format("{},{},{}\n", round(i[0]), round(i[1]), round(i[2]));
    }

    s21::Vert_t projection;
    s21::SimplePerspective proj(2, window_w, window_h);
    for (const auto& point3d : m) {
        s21::Point p = proj.project({point3d[0], point3d[1], point3d[2]});
        projection.push_back(p);
        std::cout << std::format("w{}:h{} - {},{},{} -> {},{}\n", window_w, window_h, point3d[0], point3d[1], point3d[2], p.x, p.y);
    }
    ...
    cairo_set_source_rgb(cr, 0, 0, 1);
    renderSegment(cr, {m[0][0], m[0][1], m[0][2]}, {m[1][0], m[1][1], m[1][2]});
    cairo_stroke(cr);

    cairo_set_source_rgb(cr, 1, 0, 0);
    renderSegment(cr, {m[2][0], m[2][1], m[2][2]}, {m[3][0], m[3][1], m[3][2]});
    cairo_stroke(cr);

    cairo_set_source_rgb(cr, 0, 1, 0);
    renderSegment(cr, {m[4][0], m[4][1], m[4][2]}, {m[5][0], m[5][1], m[5][2]});
    cairo_stroke(cr);
...
void SimpleCanvas::renderSegment(cairo_t* cr, const s21::Point3d& p1, const s21::Point3d& p2) {
    // Настройка камеры
    ...
    draw_line(cr, sp1.x, sp1.y, sp2.x, sp2.y);
